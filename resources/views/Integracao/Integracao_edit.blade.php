        ...
        <div class="card-body p-4">
            ...
            <form action="{{route('create.integracao')}}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="sistema" name="sistema" placeholder="nome do sistema à ser integrado" value="{{$integracao->sistema}}" required>
                </div>
                <div class="mb-3">
                    <i class="bi bi-file-earmark-text"></i><label for="descricao" class="form-label">&nbspDescrição</label>
                    <input type="text" class="form-control" id="descricao" name="descricao" placeholder="descrição da integração" value="{{$integracao->descricao ?? ''}}" required>
                </div>
                <div class="mb-3">
                    <i class="bi bi-file-earmark-text"></i><label for="slug" class="form-label">&nbspSlug</label>
                    <input type="text" class="form-control" id="slug" name="slug" placeholder="Slug (Único)" value="{{$integracao->slug}}" required>
                </div>
                <div class="mb-3">
                    <i class="bi bi-file-earmark-text"></i><label for="endpoint" class="form-label">&nbspEndpoint</label>
                    <input type="text" class="form-control" id="endpoint" name="endpoint" placeholder="endpoint da integração" value="{{$integracao->endpoint}}" required>
                </div>
                <div class="mb-3">
                    ...
                    <select name="auth" id="" class="form-control">
                        <option value="basic" {{$integracao->auth == 'basic' ? 'selected' : ''}}>basic</option>
                        <option value="bearer" {{$integracao->auth == 'bearer' ? 'selected' : ''}}>bearer</option>
                        <option value="wss" {{$integracao->auth == 'wss' ? 'selected' : ''}}>wss</option>
                    </select>
                </div>
                <div class="mb-3">
                    <label class="form-label d-block"><i class="bi bi-journal-bookmark-fill"></i>&nbspModelo da API</label>
                    <select name="tipo" id="" class="form-control">
                        <option value="soap" {{$integracao->tipo == 'soap' ? 'selected' : ''}}>Soap</option>
                        <option value="rest" {{$integracao->tipo == 'rest' ? 'selected' : ''}}>Rest</option>
                    </select>
                </div>
